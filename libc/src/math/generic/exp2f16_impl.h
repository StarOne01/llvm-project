...
LIBC_INLINE float16 exp2f16(float16 x) {
  using FPBits = typename fputil::FPBits<float16>;
  FPBits xbits(x);

  uint16_t x_u = xbits.uintval();
  uint16_t x_abs = x_u & 0x7fff'ffffU;
  ...
  if (LIBC_UNLIKELY(x_abs >= 0x4300'0000U || x_abs <= 0x3d00'0000U)) {
    ...
    if (x_abs <= 0x3d00'0000) {
      ...
      float xsq = xd * xd;
      float c0 = fputil::multiply_add(xd, COEFFS[1], COEFFS[0]);
      float c1 = fputil::multiply_add(xd, COEFFS[3], COEFFS[2]);
      float c2 = fputil::multiply_add(xd, COEFFS[5], COEFFS[4]);
      float p = fputil::polyeval(xsq, c0, c1, c2);
      float r = fputil::multiply_add(p, xd, 1.0);
